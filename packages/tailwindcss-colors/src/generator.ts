import type { ColorSystem, DarkModeConfig, GeneratorConfig, ColorVariants, SemanticColor, MaterialColor } from '@color-system/colors';

export function generateColorVariable(name: string, value: string): string {
  return `--color-${name}: ${value}`;
}

export function generateColorVariables(colors: ColorSystem, mode: 'light' | 'dark'): string {
  const lines: string[] = [];
  ...
  for (const [colorName, depthColors] of Object.entries(colors.element)) {
    for (const [depth, variants] of Object.entries(depthColors as SemanticColor)) {
      const colorValue = (variants as ColorVariants)[mode];
...
export function generateTailwindTheme(colors: ColorSystem, darkMode?: DarkModeConfig): string {
